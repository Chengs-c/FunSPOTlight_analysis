...
cell_types_A=cell_types[cell_types%in%pdac_A$annotation]
```
...
umap_pdac_b <- DimPlot(pdac_B, reduction = "umap", group.by = "annotation") + scale_color_tableau(palette = "Tableau 20")+
  scale_x_discrete(breaks=NULL)+
  scale_y_discrete(breaks=NULL)+
  theme(axis.line = element_line(arrow = arrow(length = unit(0.5, 'cm'))))
  #theme(element_blank())
...
cluster_markers_filt_a <- cluster_markers_a %>% filter(pct.2 < 0.2)
cluster_markers_filt_a <- cluster_markers_a %>% filter(pct.1 > 0.7)
cluster_markers_filt_a=cluster_markers_a
cluster_markers_filt_a %>% dplyr::count(cluster)
...
clust_vr="annotation"
plt_names <- sort(unique(pdac_A@meta.data[, clust_vr]))
...
```{r}
setwd("D:\\research\\undergraduate\\fda_ST\\true_data\\Pancreas\\plot1\\plot_A")
...
plt2=ggplot()+
  ...
  scale_fill_manual(values = as.character(df_col[as.character(df_col$plt_name) %in% cell_types_A, "col_ct"])) 
#theme_classic()  +scale_fill_manual(values = colorRampPalette(brewer.pal(19, "Pastel2"))(19))
...
ct_plt <- lapply(cell_types_A, function(ct){
  ...
  tmp_plt <- spanielPlot(object = st_se, 
    ...
    #scale_color_gradientn(colours = carto_pal(7,"OrYel"),
    #                      limits = c(0, 1), name = "Proportion") +
...
```{r}
gene=rownames(cluster_markers_filt_a[cluster_markers_filt_a$cluster==cell_types_A[[8]],])[6]
```
...
gene_plt <- lapply(cell_types_A, function(ct) {
    ...
    tmp_plt <- spanielPlot(object = st_se,
            ...
            ptSizeMax = 5, 
            ptSizeMin = 1,
            customTitle = sprintf("Expression of: %s (marker gene of Fibroblasts)",  gene)) +
      theme_classic() +
      scale_color_gradientn(
        colours = wesanderson::wes_palette("Zissou1", 100,type = "continuous"))+
      theme(
...
```{r}
cancer_prop <- rowSums(decon_mtrx[, c("Cancer.clone..S100A4.", "Cancer.clone..TM4SF1.")])
healthy_prop <- rowSums(decon_mtrx[, c("Acinar.cells", "Centroacinar.ductal.cells", "Terminal.ductal.cells", "Antigen.presenting.ductal.cells", "Tuft.cells" )])
