...
##
#data_performance = data.frame(matrix(0, nrow = (4 * num_order), ncol = 3))
#colnames(data_performance) = c("cl_n", "value", "metric")
#data_performance[, 3] = rep(c("accuracy", "sensitivity", "specificity", "F1"), num_order)

data_performance_pearson = data.frame(matrix(0, 
                                             nrow = (3 * num_repeat * num_order*n_syn_mixt),
                                             ncol = 3))
colnames(data_performance_pearson) = c("cl_n", "value", "num_repeat")
#data_performance_JSD[, 3] = rep(c("0.25", "0.50", "0.75"), num_order * num_repeat)

data_performance_RMSE = data.frame(matrix(0, nrow = (2 * num_repeat * num_order*n_syn_mixt),
                                          ncol = 4))
colnames(data_performance_RMSE) = c("cl_n", "value", "num_repeat", "metric")

for (i in 1:num_order) {
  norder = i +1
  data_performance_pearson[(1 + (i - 1) * 1000 * num_repeat):(1000*i*num_repeat), 1] = rep(norder, 1000*num_repeat)
  data_performance_RMSE[(1 + 2*(i - 1) * 1000 * num_repeat):(2*1000*i*num_repeat),1]=rep(norder, 2000*num_repeat)
